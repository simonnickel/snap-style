...
public struct StyleIcon: View {
    ...
    public init(_ icon: Style.Attribute.Icon.ValueBuilderKeyPath) {
        self.icon = .icon(icon)
    }
...
extension StyleIcon {
    public enum Definition {
        case icon(Style.Attribute.Icon.ValueBuilderKeyPath)
        case system(String)

        /// Convenience init to create a definition if either a KeyPath or String might be available.
        init?(icon: Style.Attribute.Icon.ValueBuilderKeyPath?, systemImage: String?) {
            if let icon {
                self = .icon(icon)
