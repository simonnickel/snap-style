...
public struct StyleIcon: View {
    ...
    public init(_ icon: Style.Keys.Icon.ValueBuilderKeyPath) {
        self.icon = .icon(icon)
    }
...
extension StyleIcon {
    public enum Definition {
        case icon(Style.Keys.Icon.ValueBuilderKeyPath)
        case system(String)

        /// Convenience init to create a definition if either a KeyPath or String might be available.
        init?(icon: Style.Keys.Icon.ValueBuilderKeyPath?, systemImage: String?) {
            if let icon {
                self = .icon(icon)
